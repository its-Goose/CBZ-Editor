...
class CBZEditor:
    ...
    def set_dark_theme(self):
        ...
        self.style.configure('TButton', background='#404040', foreground=
            'white')
    ...
    def create_widgets(self):
        ...
        ttk.Button(header_frame, text='←', width=3, command=self.
            load_previous_cbz).pack(side=tk.LEFT, padx=5)
        ttk.Button(header_frame, text='→', width=3, command=self.load_next_cbz
            ).pack(side=tk.RIGHT, padx=5)
        self.title_label = ttk.Label(header_frame, text='No CBZ Loaded',
            font=('Arial', 14, 'bold'))
        self.title_label.pack(pady=5, expand=True, fill=tk.X)
        control_frame = ttk.Frame(header_frame)
        control_frame.pack(pady=10)
        ttk.Button(control_frame, text='📂 Open', command=self.open_cbz).pack(
            side=tk.LEFT, padx=5)
        self.save_btn = ttk.Button(control_frame, text='💾 Save', width=15,
            style='Large.TButton', command=self.save_and_next)
        self.save_btn.pack(side=tk.LEFT, padx=5)
        ttk.Button(control_frame, text='✖ Close', command=self.close_and_next
            ).pack(side=tk.LEFT, padx=5)
        toolbar = ttk.Frame(main_frame)
        toolbar.pack(fill=tk.X, padx=5, pady=5)
        ttk.Label(toolbar, text='Thumbnail Size:').pack(side=tk.LEFT)
        self.size_slider = ttk.Scale(toolbar, from_=100, to=500, command=lambda
            e: self.update_thumbnail_size())
    ...
    def create_batch_button(self):
        batch_btn = ttk.Button(self.root, text='Batch Create CBZs', command
            =self.batch_create_cbzs)
        batch_btn.place(relx=1.0, rely=0.0, anchor='ne', x=-10, y=10)

    def batch_create_cbzs(self):
        zip_files = filedialog.askopenfilenames(title=
            'Select Zip Files with Folders', filetypes=[('Zip files',
            '*.zip')], multiple=True)
        if not zip_files:
            return
        ...
        threading.Thread(target=self.process_batch, args=(zip_files,
            output_dir, progress, status_label, progress_window), daemon=True
            ).start()

    def create_cbz_from_folder(self, folder_path, cbz_path):
        image_files = sorted([f for f in os.listdir(folder_path) if f.lower
            ().endswith(('.png', '.jpg', '.jpeg'))], key=lambda x: [(int(c) if
            c.isdigit() else c) for c in re.split('(\\d+)', x)])
    ...
    def open_cbz(self):
        ...
        progress.pack(pady=10)
        progress.start()
        threading.Thread(target=self.load_cbz_files, args=(loading_window,),
            daemon=True).start()
    ...
    def load_cbz(self, file_path):
        try:
            ...
            self.title_label.config(text=
                f'Editing: {cbz_filename} ({self.current_index + 1}/{len(self.cbz_files)})'
                )
    ...
    def initialize_file_monitor(self):
        ...
        self.file_timestamps = {f: os.path.getmtime(os.path.join(self.
            temp_dir, f)) for f in os.listdir(self.temp_dir)}
            ...
            try:
                ...
                for filename in current_files:
                    ...
                    if os.path.isfile(file_path):
                        ...
                        if (filename not in self.file_timestamps or 
                            current_mtime > self.file_timestamps[filename]):
                            self.file_timestamps[filename] = current_mtime
                            self.root.after(0, self.update_single_image,
                                file_path)
    ...
    def update_single_image(self, img_path):
        ...
        if filename in self.image_frames:
            try:
                ...
                btn_frame.pack()
                ttk.Button(btn_frame, text='Edit', command=lambda p=
                    img_path: self.edit_image(p)).pack(side=tk.LEFT)
                ttk.Button(btn_frame, text='Delete', command=lambda f=
                    filename: self.delete_image(f)).pack(side=tk.LEFT)
            except Exception as e:
                print(f'Error updating {filename}: {e}')

    def handle_deleted_file(self, filename):
        if filename in self.image_frames:
            self.image_frames[filename].grid_remove()
            del self.image_frames[filename]
    ...
    def display_images(self):
        if not self.temp_dir or not self.needs_refresh:
            return
        try:
            image_files = sorted([f for f in os.listdir(self.temp_dir) if f
                .lower().endswith(('.png', '.jpg', '.jpeg'))], key=lambda x:
                [(int(c) if c.isdigit() else c) for c in re.split('(\\d+)', x)]
                )
        except:
            image_files = sorted(os.listdir(self.temp_dir))
    ...
    def create_image_widgets(self, frame, img_path, filename):
        ...
        btn_frame = ttk.Frame(frame)
        btn_frame.pack()
        ttk.Button(btn_frame, text='Edit', command=lambda p=img_path: self.
            edit_image(p)).pack(side=tk.LEFT)
        ttk.Button(btn_frame, text='Delete', command=lambda f=filename:
            self.delete_image(f)).pack(side=tk.LEFT)
    ...
    def save_cbz(self):
        if not self.current_cbz or not self.temp_dir:
            ...
            return False
        try:
            with zipfile.ZipFile(self.current_cbz, 'w', zipfile.ZIP_DEFLATED
                ) as zipf:
    ...
    def show_status(self, message, color):
        ...
        self.root.after(2000, lambda : self.save_btn.config(style=
            'Large.TButton'))
    ...
    def process_batch(self, zip_files, output_dir, progress, status_label,
        progress_window):
        ...
        try:
                ...
                try:
                    ...
                    for root, dirs, files in os.walk(temp_dir):
                        for dir_name in dirs:
                            ...
                            cbz_path = os.path.join(output_dir,
                                f'{dir_name}.cbz')
            ...
            messagebox.showinfo('Batch Complete',
                f"""Successfully created {created_count} CBZ files in:
{output_dir}"""
                )
        except Exception as e:
            progress_window.destroy()
